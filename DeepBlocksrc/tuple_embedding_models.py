...
import random 
import warnings
warnings.filterwarnings('ignore', message="`load_model` does not return WordVectorModel or SupervisedModel any more, but a `FastText` object which is very similar.")
...
import DeepBlocksrc.dl_models as dl_models
from DeepBlocksrc.configurations import *
...
class AutoEncoderTupleEmbedding(ABCTupleEmbedding):
    ...
    def get_tuple_embedding(self, list_of_tuples):
        ...
        if torch.cuda.is_available():
            ...
            device = 'cpu'
            # device = 'mps'
        return self.autoencoder_model.get_tuple_embedding(embedding_matrix.to(device))
